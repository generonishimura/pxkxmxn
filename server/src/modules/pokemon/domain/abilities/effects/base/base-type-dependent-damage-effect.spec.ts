import { BaseTypeDependentDamageEffect } from './base-type-dependent-damage-effect';
import { BattlePokemonStatus } from '@/modules/battle/domain/entities/battle-pokemon-status.entity';
import { BattleContext } from '../../battle-context.interface';
import { Weather, BattleStatus } from '@/modules/battle/domain/entities/battle.entity';

/**
 * テスト用の具象クラス（ほのおタイプのダメージ半減）
 */
class TestFireDamageEffect extends BaseTypeDependentDamageEffect {
  protected readonly affectedTypes = ['ほのお'] as const;
  protected readonly damageMultiplier = 0.5;
}

/**
 * テスト用の具象クラス（ほのお・こおりタイプのダメージ半減）
 */
class TestMultipleTypeDamageEffect extends BaseTypeDependentDamageEffect {
  protected readonly affectedTypes = ['ほのお', 'こおり'] as const;
  protected readonly damageMultiplier = 0.5;
}

describe('BaseTypeDependentDamageEffect', () => {
  ...
  describe('modifyDamage', () => {
    it('should return modified damage for affected type', () => {
      const effect = new TestFireDamageEffect();
      const result = effect.modifyDamage(pokemon, 100, battleContext);
      expect(result).toBe(50); // 100 * 0.5 = 50
    });

    it('should return original damage for non-affected type', () => {
      const effect = new TestFireDamageEffect();
    ...
    it('should return modified damage for multiple affected types', () => {
      const effect = new TestMultipleTypeDamageEffect();
      expect(effect.modifyDamage(pokemon, 100, battleContext)).toBe(50); // ほのお
      const contextWithIce: BattleContext = {
        ...battleContext,
        moveTypeName: 'こおり',
      };
      expect(effect.modifyDamage(pokemon, 100, contextWithIce)).toBe(50); // こおり
    });
