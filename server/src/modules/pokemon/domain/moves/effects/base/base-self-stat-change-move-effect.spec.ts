import { BaseSelfStatChangeMoveEffect } from './base-self-stat-change-move-effect';
 ...
 * テスト用の具象クラス（攻撃ランク+1）
 */
class TestAttackBoostEffect extends BaseSelfStatChangeMoveEffect {
  protected readonly statType = 'attack' as const;
  protected readonly rankChange = 1;
}

describe('BaseSelfStatChangeMoveEffect', () => {
  ...
  beforeEach(() => {
    attacker = {
      ...
      getStatRank: jest.fn((statType: string) => {
        if (statType === 'attack') return attacker.attackRank;
        return 0;
      }),
  ...
  describe('onUse', () => {
    it('should increase attack rank by 1', async () => {
      const effect = new TestAttackBoostEffect();
    ...
    it('should not change rank when already at maximum', async () => {
      const attackerWithMaxRank = {
        ...attacker,
        attackRank: 6,
      } as BattlePokemonStatus;
      (attackerWithMaxRank.getStatRank as jest.Mock).mockReturnValue(6);
      const effect = new TestAttackBoostEffect();
      const result = await effect.onUse(attackerWithMaxRank, defender, battleContext);
