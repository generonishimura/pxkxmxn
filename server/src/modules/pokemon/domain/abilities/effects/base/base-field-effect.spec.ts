...
/**
 * BaseFieldEffectのテスト用の具象クラス
 */
class TestFieldEffect extends BaseFieldEffect {
  protected readonly field = Field.ElectricTerrain;
}

describe('BaseFieldEffect', () => {
  let effect: TestFieldEffect;
  let pokemon: BattlePokemonStatus;
  let battleContext: BattleContext;
  let mockBattleRepository: jest.Mocked<IBattleRepository>;

  beforeEach(() => {
    effect = new TestFieldEffect();
  ...
  describe('onEntry', () => {
    it('should change field to ElectricTerrain when onEntry is called', async () => {
    ...
    it('should not change field when already ElectricTerrain', async () => {
