...
/**
 * かちき（Competitive）特性の効果
 * 相手にステータスが下がったとき、特攻を2段階上げる
 *
 * 注意: この特性は、ターン終了時に相手のステータスが下がっているかどうかを
 * チェックする必要がある。現在の実装では、ターン終了時にこの特性をチェックする。
 */
export class CompetitiveEffect implements IAbilityEffect {
  /**
   * ターン終了時に発動
   * 相手のステータスが下がっている場合、特攻を2段階上げる
   */
  async onTurnEnd(pokemon: BattlePokemonStatus, battleContext?: BattleContext): Promise<void> {
    ...
    // 現在の特攻ランクを取得
    const currentSpecialAttackRank = pokemon.specialAttackRank;

    // 新しいランクを計算（-6から+6の範囲内で）
    const newSpecialAttackRank = Math.max(-6, Math.min(6, currentSpecialAttackRank + 2));
